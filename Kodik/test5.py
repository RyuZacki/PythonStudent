...
for begin_end in years:
    ...
    filt_data = filter_by_year(oscar_data, begin, end)  # < напишите код здесь >

    # вычислите средние значения
    mean_score = column_mean(filt_data, 2)  # < напишите код здесь >
    mean_length = column_mean(filt_data, 2)
    mean_ppm = column_mean(filt_data, 6)
    mean_gross = column_mean(filt_data, 7)

    # добавьте строку в новую таблицу
    rows.append([name, mean_score, mean_length, mean_ppm, mean_gross])  # < напишите код здесь >
